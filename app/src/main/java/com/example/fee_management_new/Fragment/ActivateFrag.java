...
//        buildSettelmentCard();
        ...
        Generatepayment_tv.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                try {
                    if (feePaidBy.equals("institute")) {
                        feepaid_by = "Processing fee payable by institute";
                    } else {
                        feePaidBy = "Processing fee payable by student";
                        Log.i(TAG, "onClick: " + feePaidBy);
                    }
                    NavDirections action1 = ActivateFragDirections.actionActivateFragToGeneratePaymentRequest(feePaidBy);
...
//        payment_Request_Activate = view.findViewById(R.id.payment_request_activate)
        withData_Cardview = view.findViewById(R.id.withdata_cardview);
        nodata_Cardview = view.findViewById(R.id.nodata_cardView);
        ...
        Generatepayment_tv = view.findViewById(R.id.GeneratePayment_tv);
        ...
        veiwmore_tv = view.findViewById(R.id.veiwMore_tv);
        Total_amt = view.findViewById(R.id.total_amt_activate);
        AddOffPayment = view.findViewById(R.id.AddOfflinepayment);
        Transaction_layout = view.findViewById(R.id.Transaction_layout);
        AllStudentLaout = view.findViewById(R.id.allStudentslayout);
        Payment_request_activate = view.findViewById(R.id.payment_request_activate);
    }
...
//                if (overViewResponse)
                ...
                try {
                    ...
                    feePaidBy = overViewResponse.getFeepayer();
                    ...
                    PaymentPendingAmount.setText(new StringBuilder().append("\u20B9 ").append(overViewResponse.totalPaymentPending.getAmount()).toString());
...
//                    buildSettelmentCard();
                    ...
                    for (AllTransactionList j : allTransactionLists) {
                        ...
                        try {
                            recentActivitiesoneArrayList.add(new RecentActivitiesone(j.getUser().getName(), j.getUser().getStudent().getStandard().getStd(), j.getUser().getStudent().getStandard().getSection(),String.valueOf(j.getAmountPayable()), j.getDate(), j.getStatus(), j.getNote(), j.getUser().getImage(), j.getId()));
                        } catch (Exception e) {
                            recentActivitiesoneArrayList.add(new RecentActivitiesone("No data", "No data", "No data", "no data", "no data", "no data", "no data", "null", 0));
                        }
...
//    }
    ...
    private void buildRecentActivity() {
        ...
        Log.i(TAG, "buildRecentActivity: " + recentActivitiesoneArrayList.size());
        adapter = new RecyclerviewRecentActivityAdapter(recentActivitiesoneArrayList, getContext());
